...
public class DriverServiceImpl implements DriverService {
    ...
    public Optional<DriverResponse> updateDriver(Long id, DriverRequest driverRequest) {
        try {
            Driver getDriver = driverRepository.findById(id).orElseThrow();

            if (getDriver.getId() != null) {
                Driver driver = driverRepository.save(
                        new Driver(
                                id,
                                driverRequest.getSurname(),
    ...
    @Override
    public Boolean uploadFileDrivers(MultipartFile file) throws IOException {
        try {
